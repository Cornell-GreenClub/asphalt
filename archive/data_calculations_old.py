...
"""
def compute_work_actual_matrix(distance_data, elevation_data, weight_data, location_to_index, w_ext):
...
def compute_work_for_route(route, distance_data, elevation_data, weight_data, index_to_location_name, w_ext=0,): #last param old: work_actual_f=0
    ...
    for i in range(len(route) - 1):
        ...
        if w_ext != 0:
            work_entry['W_AA->B'] = w_th #+ w_ext #w_th * (1 + work_actual_f)
            #print("Work Theoretical For ", index_to_location_name[start], " to ", index_to_location_name[end], " : ", w_th)

            #print("Work Actual For ", index_to_location_name[start], " to ", index_to_location_name[end], " : ", w_th + w_ext)
...
def calculate_total_work_cost_in_J(work_matrix, w_ext):
    return sum(segment['W_AA->B'] for segment in work_matrix) #sum(segment['W_AA->B'] for segment in work_matrix)
